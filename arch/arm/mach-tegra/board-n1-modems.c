...
#if defined CONFIG_SEC_MODEM_N1
#include <mach/gpio-n1.h>
#elif defined CONFIG_SEC_MODEM_BOSE
...
static struct modemlink_pm_data modem_link_pm_data = {
	...
	.gpio_link_active = GPIO_ACTIVE_STATE_HSIC,
	.gpio_link_hostwake = GPIO_IPC_HOST_WAKEUP,
	.gpio_link_slavewake = GPIO_IPC_SLAVE_WAKEUP,
	.gpio_link_flm_txd = GPIO_FLM_TXD,
	.gpio_link_flm_rxd = GPIO_FLM_RXD,
...
static struct modem_data umts_modem_data = {
	...
	.gpio_cp_dump_int = GPIO_SUSPEND_REQUEST_HSIC,	/* judge Dump mode */
	.gpio_flm_uart_sel = 0,
...
static void xmm_gpio_revers_bias_clear(void)
{
	...
	gpio_direction_output(modem_link_pm_data.gpio_link_flm_txd, 0);
	gpio_direction_output(modem_link_pm_data.gpio_link_flm_rxd, 0);
...
{
	pr_err("%s\n", __func__);
	gpio_direction_input(modem_link_pm_data.gpio_link_hostwake);
...
static void modem_link_pm_config_gpio(void)
{
	...
	unsigned gpio_link_flm_txd = modem_link_pm_data.gpio_link_flm_txd;
	unsigned gpio_link_flm_rxd = modem_link_pm_data.gpio_link_flm_rxd;
	...
	if (gpio_link_flm_txd) {
		tegra_gpio_enable(gpio_link_flm_txd);
		err = gpio_request(gpio_link_flm_txd, "FLM_TXD");
		if (err) {
			printk(KERN_ERR "fail to request gpio %s : %d\n",
			       "FLM_TXD", err);
		}
		gpio_direction_output(gpio_link_flm_txd, 0);
	}

	if (gpio_link_flm_rxd) {
		tegra_gpio_enable(gpio_link_flm_rxd);
		err = gpio_request(gpio_link_flm_rxd, "FLM_RXD");
		if (err) {
			printk(KERN_ERR "fail to request gpio %s : %d\n",
			       "FLM_RXD", err);
		}
		gpio_direction_output(gpio_link_flm_rxd, 0);
	}
