...
#define SEC_PORT_PATH_OFFSET		0x100004        /* 1MB + 4 bytes*/

static bool misc_path_operation(int offset, void *value, int size, int direction)
{
	/* from MSC(misc) partition  */
	struct file *filp;
	mm_segment_t fs;
	int ret;
	bool success = true;

	if(direction == MISC_WRITE)
		filp = filp_open(SEC_MISC_FILE_NAME, O_RDWR | O_SYNC, 0);
	else
		filp = filp_open(SEC_MISC_FILE_NAME, O_RDONLY, 0);


	//int flag = (direction == MISC_WRITE) ? (O_RDWR | O_SYNC) : O_RDONLY;
	...
	if (ret < 0) {
		printk("%s SEC_DEBUG_LEVEL FAIL LLSEEK \n", __func__);
		success = false;
		goto misc_sec_debug_out;
	}

	if (direction == MISC_READ) {
		ret = filp->f_op->read(filp, (char __user *)value,
				       sizeof(int), &filp->f_pos);
	} else if (direction == MISC_WRITE) {
...
static bool kernel_sec_set_port_path(struct kernel_sec_port_path *path) {
	return misc_path_operation(SEC_PORT_PATH_OFFSET, path, sizeof(struct kernel_sec_port_path), MISC_WRITE);
}

static bool kernel_sec_get_port_path(struct kernel_sec_port_path *path) {
	return misc_path_operation(SEC_PORT_PATH_OFFSET, path, sizeof(struct kernel_sec_port_path), MISC_READ);
}

kernel_sec_path_type kernel_sec_get_path(kernel_sec_port_type port_type) {
	struct kernel_sec_port_path port_path;
	int ret = 0;
	ret = kernel_sec_get_port_path(&port_path);
	...
	switch (port_type) {
	case SEC_PORT_UART:
		return port_path.uart_path;
	case SEC_PORT_USB:
		return port_path.usb_path;
	default:
		return SEC_PORT_PATH_ERROR;
	}
}
EXPORT_SYMBOL(kernel_sec_get_path);

bool kernel_sec_set_path(kernel_sec_port_type port_type,
			 kernel_sec_path_type path_type) {
	struct kernel_sec_port_path port_path;
	int ret = 0;
	ret = kernel_sec_get_port_path(&port_path);
	...
	case SEC_PORT_UART:
		printk("UART TO %d\n", path_type);
		port_path.uart_path = path_type;
		break;
	case SEC_PORT_USB:
		printk("USB TO %d\n", path_type);
		port_path.usb_path = path_type;
		break;
	default:
		return false;
	}

	kernel_sec_set_port_path(&port_path);

	return true;
}
EXPORT_SYMBOL(kernel_sec_set_path);
