...
static bool force_reset_timer_flag = 0;
...
static void mxt_timer_forcereset(struct work_struct *work)
{ 
	struct	mxt_data *mxt;
...
#endif
	} else {	/* since board rev.10, touch key is 4 key array */
		switch (tsp_keystatus) {
...
static int mxt_config_settings(struct mxt_data *mxt)
{ 
	...
	if (mxt->device_info.family_id == MAX224_FAMILYID) {
		tsp_config = mxt->pdata->config_224;
	}else if (mxt->device_info.family_id == MAX224E_FAMILYID) {
		tsp_config = mxt->pdata->config_224E;
	}
	
	for (i = 0; tsp_config[i][0] != MXT_END_OF_MESSAGES; i++) {

		obj_addr = MXT_BASE_ADDR(tsp_config[i][0]);
		obj_size = MXT_GET_SIZE(tsp_config[i][0]);

		error = mxt_write_block(client, obj_addr, obj_size, (u8 *)tsp_config[i]+1);

		if (error < 0) { 
			dev_err(&client->dev, "[TSP] mxt_write_block failed! (%s, %d) object = %d\n", __func__, __LINE__, tsp_config[i][0]);
			return -EIO;
		}
		
		if ((tsp_config[i][0] == MXT_SPT_CTECONFIG_T28) || (tsp_config[i][0] == MXT_SPT_CTECONFIG_T46)) {
			if (mxt->device_info.family_id == MAX224_FAMILYID) {
				mxt->xy_mode = tsp_config[i][3];
			} else if (mxt->device_info.family_id == MAX224E_FAMILYID) {
				mxt->xy_mode = tsp_config[i][2];
			}
		}
...
#ifdef CONFIG_KERNEL_DEBUG_SEC
			if (debug >= DEBUG_MESSAGES) 
				pr_info("[TSP_KEY] P %s\n", tsp_4keyname[message[MXT_MSG_T15_KEYSTATE] - 1]);
#endif
		} else { 
			switch (tsp_keystatus) {
			case TOUCH_4KEY_MENU:
...
#endif
	if(mxt->check_auto_cal == 3) {
		
		mxt->check_auto_cal = 5;
		/* T8_TCHAUTOCAL  */
/*
			error = mxt_write_byte(mxt->client,
				MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_TCHAUTOCAL,
				0);
			if (error < 0) pr_err("[TSP] %s, %d Error!!\n", __func__, __LINE__);
*/
	/* Case 4 : Calibrated from none Calibraton command*/
		...
		error = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_TCHAUTOCAL, 0);
		error = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_ATCHCALST, 0);
		error = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_ATCHCALSTHR, 0);
		error = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_ATCHFRCCALTHR, 0);
		error = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_ATCHFRCCALRATIO, 0);

		/* palm suppression Disable, 0506 LBK */
		error = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_PROCI_TOUCHSUPPRESSION_T42) + MXT_ADR_T42_CTRL, 0x05);
#endif
		if ( error < 0 ){ pr_err("[TSP][ERROR] line : %d\n", __LINE__);
		} else {        
			if (debug >= DEBUG_INFO)
...
{ 
	...
	if (message[MXT_MSG_T42_STATUS] == MXT_MSGB_T42_TCHSUP) { 
		...
		// 0506 LBK
		if (facesup_message_flag && timer_flag) return;
#if 0		
		error = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_ATCHCALST, 4);
		error = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_ATCHCALSTHR, 1);
		error = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_ATCHFRCCALTHR, 127);
		error = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_ATCHFRCCALRATIO, 127);
#endif
		check_chip_palm(mxt);
...
#if 0
		error = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_ATCHCALST, 250);
		error = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_ATCHCALSTHR, 8);
		error = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_ATCHFRCCALTHR, 50);
		error = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_ATCHFRCCALRATIO, 0);
#endif
...
#if 0//defined(MXT_FACTORY_TEST) || defined(MXT_FIRMUP_ENABLE)
...
{ 
	...
	do { 
		...
		if (i == 0) {
			dev_err(&client->dev, "[TSP] Failure reading maxTouch device 10 times!! 1 second later, touch ic will be reset!!\n");
			kfree(message);

			dev_err(&client->dev, "[TSP] request_threaded_irq(IRQF_DISABLED)!!\n");
			disable_irq_nosync(mxt->client->irq);
			schedule_delayed_work(&mxt->timer_dwork, 100);
			force_reset_timer_flag = true;
			return;
		}
...
static int calibrate_chip(struct mxt_data *mxt)
{
	...
	/* change calibration suspend settings to zero until calibration confirmed good */
	//ret = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_TCHAUTOCAL, 0);
	...
	if (mxt->device_info.family_id == MAX224E_FAMILYID) {
	...
	/* palm suppression Disable */
	//ret = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_PROCI_TOUCHSUPPRESSION_T42) + MXT_ADR_T42_CTRL, 0x05);
	if ( ret < 0 ) {
	...
	if((data_buffer[0] == 0xF3) && (data_buffer[1] == 0x00)) {
		...
		if (mxt->device_info.family_id == MAX224_FAMILYID) {
			...
			if((tch_ch) && (atch_ch == 0)) {
				...
				klogi_if("[TSP] calibration was not decided yet\n");
				cal_check_flag = 1u;
				if (timer_flag == DISABLE) {
					/* 100ms timer enable */
				...
				if((tch_ch == 0) && (atch_ch == 0)) {
					not_yet_count = 0;
				} else if(not_yet_count >= 100) {
					klogi_if("[TSP] not_yet_count over 100\n");
					not_yet_count =0;
					//calibrate_chip(mxt);
					/* 100ms timer disable */
					timer_flag = DISABLE;
...
static void cal_maybe_good(struct mxt_data *mxt)
{
	...
	if(timer_flag == ENABLE)
	{
		...
		if(timer_ticks >= 3) { 
			...
			ret = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_ATCHCALST, 8);
			ret = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_ACQUIRECONFIG_T8) + MXT_ADR_T8_ATCHCALSTHR, 8);
			...
			/* palm suppression Enable */
			ret = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_PROCI_TOUCHSUPPRESSION_T42) + MXT_ADR_T42_CTRL, T42_CTRL);

			/* MXT_ADR_T9_TCHTHR */
			ret = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_TOUCH_MULTITOUCHSCREEN_T9) + MXT_ADR_T9_TCHTHR, 30);
#endif
...
retry_i2c:
	...
	/* Check Family Info */
	if ((mxt->device_info.family_id == MAX224_FAMILYID) || (mxt->device_info.family_id == MAX224E_FAMILYID)) { 
...
{ 
	...
	timer_ticks = 0;
	ts_100ms_timer_stop(mxt);
	//mxt_reconfig_flag = false;
	if (force_reset_timer_flag == true) {
...
param_check_ok:
	...
	/* TSP Power on */
	if (!IS_ERR(mxt->pdata->reg_vdd)) {
		regulator_enable(mxt->pdata->reg_vdd);
	...
	if (!IS_ERR(mxt->pdata->reg_avdd)) {
		regulator_enable(mxt->pdata->reg_avdd);
	...
	if (!IS_ERR(mxt->pdata->reg_vdd_lvsio)) {
		regulator_enable(mxt->pdata->reg_vdd_lvsio);
	...
	INIT_DELAYED_WORK(&mxt->timer_dwork, mxt_timer_forcereset); 
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	mxt->early_suspend.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 1;
