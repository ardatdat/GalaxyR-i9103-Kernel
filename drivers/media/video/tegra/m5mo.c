	...
	(struct m5mo_info *info, struct m5mo_mode *mode)
{
	...
	if(cur_cammod) {
		if (mode->xres == 1280 && mode->yres == 720)
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Preview_Start_Camcorder_1280_720, NULL);
		else if (mode->xres == 800 && mode->yres == 480)
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Preview_Start_Camcorder_800_480, NULL);		
		else if (mode->xres == 720 && mode->yres == 480)
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Preview_Start_Camcorder_720_480, NULL);
		else if (mode->xres == 640 && mode->yres == 480) 
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Preview_Start_Camcorder_640_480, NULL);
		else if (mode->xres == 528 && mode->yres == 432) 
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Preview_Start_Camcorder_528_432, NULL);			
		else if (mode->xres == 320 && mode->yres == 240)
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Preview_Start_Camcorder_320_240, NULL);
		else if (mode->xres == 176 && mode->yres == 144)
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Preview_Start_Camcorder_176_144, NULL);
		else {
		...
		if (mode->xres == 1280 && mode->yres == 720)
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Preview_Start_Camera_1280_720, NULL);	
		else if (mode->xres == 800 && mode->yres == 480)
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Preview_Start_Camera_800_480, NULL);
		else if (mode->xres == 720 && mode->yres == 480) 
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Preview_Start_Camera_720_480, NULL);		
		else if (mode->xres == 640 && mode->yres == 480) 
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Preview_Start_Camera_640_480, NULL);
		else if (mode->xres == 528 && mode->yres == 432) 
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Preview_Start_Camera_528_432, NULL);
		else if (mode->xres == 320 && mode->yres == 240) 
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Preview_Start_Camera_320_240, NULL);
		else if (mode->xres == 176 && mode->yres == 144) 
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Preview_Start_Camera_176_144, NULL);		
		else {
			pr_err("%s: invalid preview resolution supplied to set mode %d %d\n",
					__func__, mode->xres, mode->yres);
			return -EINVAL;
	...
	(struct m5mo_info *info, struct m5mo_mode *mode)
{
	...
	if(sensor_mode != MONITOR_MODE) {
		...
		if (mode->xres == 3264 && mode->yres == 2448)
			...
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Capture_3264x1968, NULL);
		else if (mode->xres == 2048 && mode->yres == 1536)
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Capture_2048x1536, NULL);
		else if (mode->xres == 2048 && mode->yres == 1232)
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Capture_2048x1232, NULL);
		else if (mode->xres == 800 && mode->yres == 480)
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Capture_800x480, NULL);
		else if (mode->xres == 640 && mode->yres == 480)
			m5mo_write_table_Isp(info, SetModeSequence_ISP_Capture_640x480, NULL);
		else {
	...
	(struct m5mo_info *info, enum m5mo_focus_mode arg)
{
	...
	if(info->focusmode == FOCUS_FACE_DETECT && arg != FOCUS_FACE_DETECT) {
		m5mo_write_table_Isp(info, mode_focus_face_detect_disable, NULL);
	}
	...
	case AF_STOP: 
		...
		if (!info->touchaf_enable) {
			...
			if (sysmode == 0x02) {/* if ISP state is monitor */
				err = m5mo_write_table_Isp(info, aeawb_unlock,
								NULL);
				if (err) {
...
static DEVICE_ATTR(camerafw, 0666, camerafw_file_cmd_show, camerafw_file_cmd_store);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (value == FACTORY_FLASH_OFF) {
		...
		m5mo_write_table_Isp(info, mode_flash_off, NULL);
		info->pdata->power_off();
		tegra_camera_disable_vi();
		tegra_camera_disable_csi();
	} else {
		...
		m5mo_write_table_Isp(info, mode_isp_start, NULL);
		m5mo_write_table_Isp(info, mode_flash_torch, NULL);		
	}
...
static DEVICE_ATTR(camtype, 0666, camtype_file_cmd_show, camtype_file_cmd_store);
#endif
