...
#define CONFIG_SAMSUNG_MHL_HPD_SOLUTION 1
...
#define TX_DEBUG_PRINT(fmt, ...)	do{ printk ("[MHL] %s() " fmt,  __func__, ##__VA_ARGS__); }while(0);
...
extern void sii9234_cfg_power(bool on); // dshadow
...
extern void rcp_uevent_report(u8 key);
extern	void Sii9234_int_irq_enable();
...
Bool SiI9234_startTPI(void)
{
	...
	{
		TX_DEBUG_PRINT("######## Silicon Device Id: %x\n", devID);
		return TRUE;
	}

	TX_DEBUG_PRINT("Unsupported TX\n");
	return FALSE;
}
...
{
	Bool status;
	TX_DEBUG_PRINT("# SiI9234 initialization start~ \n");
	//SiI9234_HW_Reset();
	SiiMhlTxInitialize();

	Sii9234_int_irq_enable();
	return TRUE;
}
...
void 	SiiMhlTxDeviceIsr( void )
{
	...
	{
		TX_DEBUG_PRINT(KERN_INFO	"fwPowerState=%d\n", fwPowerState);
		//
		...
		if(tmp < 0) {
			TX_DEBUG_PRINT(KERN_ERR	"read CBUS(0x08) fail(%d)\n", tmp);
			return;
		}
		...
		if(tmp < 0) {
			TX_DEBUG_PRINT(KERN_ERR	"read CBUS(0x1E) fail(%d)\n", tmp);
			return;
		}
	...
	if( enable )
	{
		...
		{//3355
			...
			if(vbus_mhl_est_fail == TRUE)
			{
				...
				{
					TX_DEBUG_PRINT(KERN_INFO	"rsen == %d \n", rsen);
					vbus_mhl_est_fail = FALSE;
				}
			} else
				TX_DEBUG_PRINT(KERN_INFO	"vbus_mhl_est_fail is fail \n");
		}//3355
	}
	else
	{
...
//					mhl_cable_status = MHL_TV_OFF_CABLE_CONNECT;
			...
			{
				TX_DEBUG_PRINT(KERN_ERR "MHL Cable disconnect### 2\n");
				mhl_cable_status =MHL_INIT_POWER_OFF;
				FSA9480_MhlSwitchSel(0);
		...
		else if(rsen == 0x04)
		{
			...
			TX_DEBUG_PRINT("MHL Cable Connection ###\n");
		}
	...
	if(reg71 & BIT_6)
	{
		...
		if((BIT_6 & cbusInt) != dsHpdStatus)
		{
			...
			else
			{
				...
				//TX_DEBUG_PRINT("<0> mhl_hpd_handler finished\n");
				dsHpdStatus=1;
				#endif
			}
...
void	ProcessRgnd( void )
{
	...
	else
	{
		...
		if(0x01==reg99RGNDRange)
		{
			...
			mhl_cable_status =MHL_TV_OFF_CABLE_CONNECT;
			TX_DEBUG_PRINT(KERN_ERR "MHL Connection Fail Power off ###1\n");
			//FSA9480_MhlSwitchSel(0);
...
void	SwitchToD3( void )
{
	...
	ReleaseUsbIdSwitchOpen();
	TX_DEBUG_PRINT("TX_POWER_STATE_D3\n");
...
static	void	Int4Isr( void )
{
	...
	// Sii9244's i2c module isn't replay switching D2 mode. so add 'mdelay'.
	do {
		count ++;
		if( (TX_POWER_STATE_D3 == fwPowerState) || (count > 0) )
			mdelay(50);
		reg74 = I2C_ReadByte(SA_TX_Page0_Primary, (0x74)); // read status
		TX_DEBUG_PRINT("REG74 : 0x%x, fwPowerState=%d, mhl_cable_status=%d, mhl_vbus=%d\n",(int)reg74, fwPowerState, mhl_cable_status, mhl_vbus);
	 }while( (reg74 & BIT_7) && (count < 3) );

	 // When I2C is inoperational (say in D3) and a previous interrupt brought us here, do nothing.
	 if(reg74 & BIT_7)
	 {
		TX_DEBUG_PRINT(KERN_ERR "[ERROR] (REG74 : 0x%x)\n", (int)reg74);
		return;
	 }
	...
	{
		TX_DEBUG_PRINT(KERN_INFO	"MHL Mode Established Interrupt\n");
		/*
		if((I2C_ReadByte(SA_TX_Page0_Primary, 0x09) & BIT_2) == 0x00) {
			TX_DEBUG_PRINT(KERN_ERR "[MHL] RSEN is low - status incorrect\n");
	...
	{
		TX_DEBUG_PRINT(KERN_INFO	"MHL Discovery Failed Interrupt\n");

		if(mhl_cable_status == (MHL_1K_IMPEDANCE_VERIFIED|MHL_POWER_ON))//|MHL_RSEN_VERIFIED))
		{
			mhl_cable_status =MHL_TV_OFF_CABLE_CONNECT;
			TX_DEBUG_PRINT(KERN_ERR "MHL Connection Fail Power off ###2\n");
			//FSA9480_MhlSwitchSel(0);
			//3355
			if(mhl_vbus == true)
			{
		...
		{
			TX_DEBUG_PRINT(KERN_INFO	"mhl_cable_status=0x%x\n", mhl_cable_status);
			MhlTxDrvProcessDisconnection();
		}
		return;
	}
			...
			case	MHL_MSC_MSG_RCP:
				...
				if(MHL_LOGICAL_DEVICE_MAP & rcpSupportTable [mhlTxConfig.mscMsgData] )
				{
					...
					*eventParameter = mhlTxConfig.mscMsgData; // key code
					rcp_uevent_report(mhlTxConfig.mscMsgData);
					TX_DEBUG_PRINT("Key Code:%x \n", (int)mhlTxConfig.mscMsgData);
				}
				else
				{
					TX_DEBUG_PRINT("Key Code Error:%x \n",(int)mhlTxConfig.mscMsgData);
...
static	void	MhlTxDriveStates( void )
{
	...
	switch( mhlTxConfig.mscState )
	{
		case MSC_STATE_BEGIN:
			TX_DEBUG_PRINT("MSC_STATE_BEGIN \n");
		...
		case MSC_STATE_POW_DONE:
			...
			//
			TX_DEBUG_PRINT("MSC_STATE_POW_DONE \n");
...
void	AppRcpDemo( byte event, byte eventParameter)
{
	...
	TX_DEBUG_PRINT("App: Got event = %02X, eventParameter = %02X\n", (int)event, (int)eventParameter);
	...
	switch( event )
	{
		...
		case	MHL_TX_EVENT_RCP_READY:
			...
			TX_DEBUG_PRINT("App: Got event = MHL_TX_EVENT_RCP_READY...Sending RCP (%02X)\n", (int) rcpKeyCode);
			break;
      ...
      switch(eventParameter)
      {
		...
		case	MHL_TX_EVENT_RCPK_RECEIVED:
			TX_DEBUG_PRINT(KERN_INFO	"App: Received an RCPK = %02X\n", (int)rcpKeyCode);
			break;

		case	MHL_TX_EVENT_RCPE_RECEIVED:
			TX_DEBUG_PRINT(KERN_INFO	"App: Received an RCPE = %02X\n", (int)rcpKeyCode);
			break;
...
void	AppVbusControl( bool powerOn )
{
	if( powerOn )
	{
		TX_DEBUG_PRINT("App: Peer's POW bit is set. Turn the VBUS power OFF here.\n");
	}
	else
	{
		TX_DEBUG_PRINT("App: Peer's POW bit is cleared. Turn the VBUS power ON here.\n");
	}
}
  ...
  //TX_DEBUG_PRINT("Start PinTxInt Pin Init : %d \n",(int)PinTxInt);
	do
	{
		...
		mhl_cable_status) //NAGSM_Android_SEL_Kernel_Aakash_20101214
		{
			...
			TX_DEBUG_PRINT("#1 (0x74) flag: %x\n",(int) flag );

			//I2C_WriteByte(SA_TX_Page0_Primary, (0x74), tmp);   // clear all interrupts
			tmp = I2C_ReadByte(SA_TX_Page0_Primary, 0x71);
			//I2C_WriteByte(SA_TX_Page0_Primary, 0x71, tmp);
			flag |= (tmp&INTR_1_DESIRED_MASK);
			TX_DEBUG_PRINT("#1 (0x71) flag: %x\n",(int) flag );

			if(mhlTxConfig.mhlConnected == MHL_TX_EVENT_DISCONNECTION)//(mhlTxConfig_status()== MHL_TX_EVENT_DISCONNECTION)//
			{
				tmp = ReadByteCBUS(0x08);
				TX_DEBUG_PRINT("#2 (ReadByteCBUS(0x08)) Temp: %x\n",(int) tmp );
				WriteByteCBUS(0x08, tmp);
				tmp = ReadByteCBUS(0x1E);
				TX_DEBUG_PRINT("#2 (ReadByteCBUS(0x1E)) Temp: %x\n",(int) tmp );
				WriteByteCBUS(0x1E, tmp);
			...
			else
			{
				...
				TX_DEBUG_PRINT("#1 (ReadByteCBUS(0x08)) Temp: %x\n",(int) flag );
				tmp = ReadByteCBUS(0x1E);
				flag |= (tmp&INTR_CBUS2_DESIRED_MASK);
				TX_DEBUG_PRINT("#1 (ReadByteCBUS(0x1E)) Temp: %x\n",(int) flag );
			}
			if((flag == 0xFA)||(flag == 0xFF))
			flag = 0;
		}
	}while(flag);
    TX_DEBUG_PRINT("#$$$$$$$$$$$$$$$ flag: %x\n",(int) flag );
}
