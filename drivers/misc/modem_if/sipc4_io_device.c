	...
	*/
	...
	case IPC_RAMDUMP:
		...
		if (in_interrupt())
			...
			skb = rx_alloc_skb(len, GFP_KERNEL, iod, ld);
		if (!skb) {
...
static long misc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_MODEM_GOTA_START:
		mif_err("[IOD] misc_ioctl : IOCTL_MODEM_GOTA_START\n");
		return ld->gota_start(ld, iod);

	case IOCTL_MODEM_FW_UPDATE:
		mif_err("[IOD] misc_ioctl : IOCTL_MODEM_FW_UPDATE\n");
		return ld->modem_update(ld, iod, arg);
