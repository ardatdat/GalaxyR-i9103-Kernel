...
int get_MPUReg(struct mldl_cfg *mldl_cfg, void *gyro_handle)
{
	...
	printk(KERN_INFO "%s : Register(0x61) = 0x%x\n", __func__, b);
	result = MLSLSerialRead(gyro_handle, mldl_cfg->addr,
				MPUREG_PWR_MGM, 1, &b);
	printk(KERN_INFO "%s : Register(0x62) = 0x%x\n", __func__, b);
#endif
 ...
 */
static int MLDLGetSiliconRev(struct mldl_cfg *pdata, void *mlsl_handle)
{
	int result;
	...
	result = MLSLSerialReadMem(mlsl_handle,
				pdata->addr, memAddr, 1, &index);
	ERROR_CHECK(result);
	...
	result =
	    MLSLSerialWriteSingle(mlsl_handle, pdata->addr, MPUREG_BANK_SEL, 0);
	ERROR_CHECK(result);
...
**/
static int
MLDLSetLevelShifterBit(struct mldl_cfg *pdata,
		       void *mlsl_handle, unsigned char enable)
{
#ifndef M_HW
	int result;
...
mpu60xx_pwr_mgmt(struct mldl_cfg *pdata,
		 void *mlsl_handle,
		 unsigned char reset, unsigned char powerselection)
{
	unsigned char b;
	...
	result =
	    MLSLSerialRead(mlsl_handle, pdata->addr, MPUREG_PWR_MGMT_1, 1, &b);
	ERROR_CHECK(result);
 ...
 */
static unsigned char
MLDLStandByGyros(struct mldl_cfg *pdata,
		 void *mlsl_handle,
		 ...
		 unsigned char disable_gy, unsigned char disable_gz)
{
	...
	result =
	    MLSLSerialRead(mlsl_handle, pdata->addr, MPUREG_PWR_MGMT_2, 1, &b);
	ERROR_CHECK(result);
 ...
 */
static unsigned char
MLDLStandByAccels(struct mldl_cfg *pdata,
		  void *mlsl_handle,
		  unsigned char disable_ax,
		  unsigned char disable_ay, unsigned char disable_az)
{
	unsigned char b;
	unsigned char result;
 ...
 */
static int
MLDLPowerMgmtMPU(struct mldl_cfg *pdata,
		 void *mlsl_handle,
		 unsigned char reset,
		 unsigned char sleep,
		 unsigned char disable_gx,
		 unsigned char disable_gy, unsigned char disable_gz)
{
	...
	if ((b & (BIT_SLEEP | BIT_STBY_XG | BIT_STBY_YG | BIT_STBY_ZG)) ==
	    (((sleep != 0) * BIT_SLEEP) |
	     ...
	     ((disable_gy != 0) * BIT_STBY_YG) | ((disable_gz !=
						   0) * BIT_STBY_ZG))) {
		return ML_SUCCESS;
	}
...
int
mpu_set_slave(struct mldl_cfg *mldl_cfg,
	      void *gyro_handle,
	      ...
	      struct ext_slave_platform_data *slave_pdata)
{
	...
	result = MLSLSerialWriteSingle(gyro_handle,
				       mldl_cfg->addr, MPUREG_I2C_SLV0_CTRL,
				       reg);
#else
	/* Length */
	result = MLSLSerialRead(gyro_handle, mldl_cfg->addr,
...
static int gyro_resume(struct mldl_cfg *mldl_cfg, void *gyro_handle)
{
	...
	/* Wake up the part */
#ifdef M_HW
	result = mpu60xx_pwr_mgmt(mldl_cfg, gyro_handle, RESET, WAKE_UP);
	ERROR_CHECK(result);
 ...
 */
int
mpu3050_open(struct mldl_cfg *mldl_cfg,
	     void *mlsl_handle,
	     void *accel_handle, void *compass_handle, void *pressure_handle)
{
	int result;
	 ...
	 */
#ifdef M_HW
	result = mpu60xx_pwr_mgmt(mldl_cfg, mlsl_handle, RESET, WAKE_UP);
#else
	...
	/* Configure the MPU */
#ifdef M_HW
	result = mpu60xx_pwr_mgmt(mldl_cfg, mlsl_handle, FALSE, SLEEP);
#else
	result = MLDLPowerMgmtMPU(mldl_cfg, mlsl_handle, 0, SLEEP, 0, 0, 0);
#endif
	ERROR_CHECK(result);
 ...
 */
int
mpu3050_close(struct mldl_cfg *mldl_cfg,
	      void *mlsl_handle,
	      void *accel_handle, void *compass_handle, void *pressure_handle)
{
	int result = ML_SUCCESS;
 ...
 */
int
mpu3050_resume(struct mldl_cfg *mldl_cfg,
	       void *gyro_handle,
	       void *accel_handle,
	       void *compass_handle,
 ...
 */
int
mpu3050_suspend(struct mldl_cfg *mldl_cfg,
		void *gyro_handle,
		void *accel_handle,
		void *compass_handle,
		void *pressure_handle,
		bool suspend_gyro,
		bool suspend_accel, bool suspend_compass, bool suspend_pressure)
{
	int result = ML_SUCCESS;
...
#else
		result =
		    MLDLPowerMgmtMPU(mldl_cfg, gyro_handle, 0, SLEEP, 0, 0, 0);
#endif
		ERROR_CHECK(result);
	}
	...
	if (!mldl_cfg->accel_is_suspended && suspend_accel &&
		...
		if (!mldl_cfg->gyro_is_suspended &&
		    EXT_SLAVE_BUS_SECONDARY == mldl_cfg->pdata->accel.bus) {
			result =
			    mpu_set_slave(mldl_cfg, gyro_handle, NULL, NULL);
			ERROR_CHECK(result);
		}
#if 0
 ...
 */
int
mpu3050_read_accel(struct mldl_cfg *mldl_cfg,
 ...
 */
int
mpu3050_read_compass(struct mldl_cfg *mldl_cfg,
 ...
 */
int
mpu3050_read_pressure(struct mldl_cfg *mldl_cfg,
...
int
mpu3050_config_accel(struct mldl_cfg *mldl_cfg,
		     void *accel_handle, struct ext_slave_config *data)
{
...
int
mpu3050_config_compass(struct mldl_cfg *mldl_cfg,
		       void *compass_handle, struct ext_slave_config *data)
{
...
int
mpu3050_config_pressure(struct mldl_cfg *mldl_cfg,
			void *pressure_handle, struct ext_slave_config *data)
{
...
int
mpu3050_get_config_accel(struct mldl_cfg *mldl_cfg,
			 void *accel_handle, struct ext_slave_config *data)
{
...
int
mpu3050_get_config_compass(struct mldl_cfg *mldl_cfg,
			   void *compass_handle, struct ext_slave_config *data)
{
...
int
mpu3050_get_config_pressure(struct mldl_cfg *mldl_cfg,
			    void *pressure_handle,
