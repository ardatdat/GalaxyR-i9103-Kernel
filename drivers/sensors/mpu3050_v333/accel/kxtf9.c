...
static int kxtf9_verify_i2caddr(void *mlsl_handle,
	struct ext_slave_descr *slave,
	struct ext_slave_platform_data *pdata)
{
	int result = ML_SUCCESS;
	unsigned char tf9_addr[] = {0xF, 0xE, 0xD, 0xC};
	unsigned char tmp_addr;
	unsigned char buf;
	int i;

	for (i = 0; i < 4; i++) {
		pr_info("%s: #%d: try to access with 0x%x slave address\n",
	...
	pr_info("%s: temporary addr = 0x%02x\n", __func__, tmp_addr);

	/* Release Soft-Reset Command */
			...
			struct ext_slave_platform_data *pdata)
{
	...
	if (kxtf9_verify_i2caddr(mlsl_handle, slave, pdata) != ML_SUCCESS)
		return result;
