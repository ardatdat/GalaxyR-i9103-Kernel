...
static void cm3663_light_enable(struct cm3663_data *cm3663)
{
	...
	hrtimer_start(&cm3663->light_timer,
		ns_to_ktime(ktime_to_ns(cm3663->light_poll_delay)+100000000),
		HRTIMER_MODE_REL);
}
				      ...
				      const char *buf, size_t size)
{
	...
	printk(KERN_INFO "proximity_enable_store : %d\n", new_value);
...
static DEVICE_ATTR(proximity_avg, 0664,
...
{
	int als;
	struct cm3663_data *cm3663 = container_of(work,
			struct cm3663_data, work_light);
			  ...
			  const struct i2c_device_id *id)
{
	...
	printk(KERN_INFO "CM3663 probe start!\n");
